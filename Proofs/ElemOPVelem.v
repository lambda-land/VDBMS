...
Lemma nodup_remove_cons a l: In a l -> nodup string_eq_dec l 
      =set= (a :: nodup string_eq_dec (remove string_eq_dec a l)).
Proof. 
...
nodup string_eq_dec (configVElemSet v c) =set= (configVElemSet (nodupelem v) c).
Proof.
...
Lemma nodup_equiv A A': A =set= A' -> 
      nodup string_eq_dec A =set= nodup string_eq_dec A'.
...
Lemma nodupelem_equiv A A': A =vset= A' -> 
      nodupelem A =vset= nodupelem A'.
