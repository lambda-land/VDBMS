...
configVElemSet (set_add velem_eq_dec (ae a f) A) c =a=  (a :: (configVElemSet A c)).
Proof. induction A.
...
configVElemSet (set_add velem_eq_dec (ae a f) A) c =a=  (configVElemSet A c).
Proof. induction A.
- intros. simpl. rewrite H0. reflexivity.
- intros. simpl in H. apply Classical_Prop.not_or_and in H.
destruct H. simpl set_add. 
...
Lemma notIn_set_add_equiv_velems a A: ~ In a A -> 
      (set_add velem_eq_dec a A) =va= (a::A) .
...
Lemma elems_union_nil_r: forall A, elems_union A [] =a= A.
Proof. intros. simpl. reflexivity. Qed.

(* elems_union use set_add; thus bag(Dup) A will become set A on LHS*)
Lemma elems_union_nil_l: forall A (H: NoDup A), elems_union [] A =a= A.
...
Lemma velems_union_nil_l : forall A (H: NoDupElem A), velems_union [] A =va= A.
Proof. 
induction A. simpl. 
reflexivity. simpl.  
intro H.
...
Lemma elems_inter_nil_l_equiv A B (HA: NoDup A) (HB: NoDup B): A =a= [] ->
elems_inter A B =a= []. 
...
Lemma elems_inter_nil_r_equiv A B (HA: NoDup A) (HB: NoDup B): B =a= [] ->
elems_inter A B =a= []. 
...
velems_inter (velems_inter A B) B =va= velems_inter A B. 
Proof. induction A.
...
elems_inter (elems_inter A B) B =a= elems_inter A B. 
Proof. induction A.
