...
-- l₁ appropriately! 
pushOutRProj (RProj as1 (RProj as2 q)) = undefined
...
selRDistr q@(RSel (SqlAnd c1 c2) (RJoin q1 q2 c)) s 
...
selRDistr q@(RSel c1 (RJoin q1 q2 c2)) s
  -- σ c₁ (q₁ ⋈\_c₂ q₂) ≡ (σ c₁ q₁) ⋈\_c₂ q₂
  | check c1 t1
    = RJoin (RSel c1 (selRDistr q1 s))
            (selRDistr q2 s)
            c2
  -- σ c₁ (q₁ ⋈\_c₂ q₂) ≡ q₁ ⋈\_c₂ (σ c₁ q₂)
  | check c1 t2
    = RJoin (selRDistr q1 s)
            (RSel c1 (selRDistr q2 s))
            c2
  | otherwise = q 
    where
      check cond env = (not (notInCond cond) && inAttInEnv cond env)
