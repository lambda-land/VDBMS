  ...
  [ testGroup "1) test simple query to V-query"
    ...
    ,
      ...
      do output    <- return $ variationizeQuery [testq1]
         expectVal <- return $ Proj [(v1,Attribute {attributeName = "A1"})] 
                               (AChc v1 (TRef (Relation {relationName = "T1"})) Empty)
         expectVal @=? output
    ,
      testCase "fold a single Query with condition  (SELECT A1 FROM T2 WHERE A2 > 5)" $
      do output    <- return $ variationizeQuery [testq2]
         expectVal <- return $ Proj [(v1,Attribute {attributeName = "A2"})] 
                               (Sel (C.CChc v1 (C.Comp GT (C.Attr (Attribute {attributeName = "A2"})) (C.Val (SqlInt32 5))) (C.Lit True)) 
                               (AChc v1 (TRef (Relation {relationName = "T2"})) Empty))
         expectVal @=? output
    ,
      testCase "fold two query with different product and different projection " $
      do output    <- return $ variationizeQuery [testq1, testq2]
         expectVal <- return $ Proj [(v1,Attribute {attributeName = "A1"}),(v2,Attribute {attributeName = "A2"})] 
                               (Sel (C.CChc v2 (C.Comp GT (C.Attr (Attribute {attributeName = "A2"})) (C.Val (SqlInt32 5))) (C.Lit True)) 
                               (AChc v2 (TRef (Relation {relationName = "T2"})) (AChc v1 (TRef (Relation {relationName = "T1"})) Empty)))
         expectVal @=? output
    ,
      testCase "fold two query with same product and common attributes" $
      do output    <- return $ variationizeQuery [testq2, testq2']
         expectVal <- return $ Proj [(v2,Attribute {attributeName = "A1"}),(v1 `Or` v2,Attribute {attributeName = "A2"})] 
                               (Sel (C.CChc (v1 `Or` v2) (C.Comp GT (C.Attr (Attribute {attributeName = "A2"})) (C.Val (SqlInt32 5))) (C.Lit True)) 
                               (AChc (v1 `Or` v2) (TRef (Relation {relationName = "T2"})) Empty))
         expectVal @=? output
    ,
      ...
      do output    <- return $ variationizeQuery [testq3,testq4] 
         expectVal <- return $ SetOp Prod 
                                 (Proj [(v2,Attribute {attributeName = "A1"}),(v1,Attribute {attributeName = "A3"})] 
                                  (AChc v2 (TRef (Relation {relationName = "T2"})) (AChc v1 (TRef (Relation {relationName = "T3"})) Empty))) 
                                 (Proj [(v2,Attribute {attributeName = "A1"}),(v1,Attribute {attributeName = "A3"})] 
                                  (AChc (v1 `Or` v2) (TRef (Relation {relationName = "T3"})) Empty))
         expectVal @=? output 
